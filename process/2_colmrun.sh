...
for year in {2001..2017}; do  # 这里你可以更改年份范围
    ...
    for mon in {3..8}; do
        ...
        # 拼接输入和输出文件路径
        input_name="${base_input_path}${year}_gravel/CoLMrun/unstructured_cwrf_CN_15km_gravel/history/unstructured_cwrf_CN_15km_gravel_hist_${year}-${mon_str}_remap.nc"
        output_name="${base_output_path}colmrun_${year}-${mon_str}_gravel.nc"
...
cdo mergetime $out_list add_colmrun_2001-2017_monmean_gravel.nc
rm $out_list
