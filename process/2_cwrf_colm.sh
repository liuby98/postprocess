...
base_output_path="/share/home/dq135/draw/scripts/temp/"
...
for year in {2001..2023}; do  # 这里你可以更改年份范围
    ...
    for mon in {3..5}; do
        ...
        for day in $(seq 1 $days_in_month); do
            ...
            # 拼接输入和输出文件路径
            input_name="${base_input_path}${year}_nogravel/wrfout_d01_${year}-${mon_str}-${day_str}_00:00:00"
            output_name="${base_output_path}wrfout_${year}-${mon_str}-${day_str}_nogravel.nc"
...
# cdo mergetime $out_list wrfout_2001-2023_JJA_daymean_nogravel.nc
# rm $out_list
