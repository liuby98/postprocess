    ...
    for row_idx, season in enumerate(seasons):
        ...
        if TARGET_VAR == 't2m':
            ...
            for col in range(3):
                ...
                cf = ax.contourf(lon_grid, lat_grid, plot_data, levels=tcc_levels[col], cmap=tcc_cmaps[col], extend='both', transform=ccrs.PlateCarree(), zorder=1, antialiased=False)
                try:
                    for c_ in cf.collections:
                        c_.set_edgecolor("face")
                        c_.set_linewidth(1e-8)
                ...
                if col < 2:
                    ...
                    lat_sig = lat_grid[final_sig_mask]
                    if len(lon_sig) > 0:
                        ax.scatter(lon_sig, lat_sig, s=8.0, color='white', marker='o', edgecolors='none', alpha=0.9, transform=ccrs.PlateCarree(), zorder=3)
