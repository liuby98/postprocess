...
for r, season in enumerate(seasons_code):
    ...
    # 4. DIFF (EXP - CTL) 绝对降水量差值 & 偏差百分比差值 (新增加权均值)
    ...
    print(f"  [{s_name}] 降水特征统计 (已统一扣除 25-29.5N, 91-98E 区域):")
    ...
    print(f"    - OBS (华南 105-115°E, 20-25°N) -> [最值: {obs_min_sc:.2f} ~ {obs_max_sc:.2f} | 均值: {obs_mean_sc:.2f}] mm/day")
    print("")
    print(f"    - CTL CPL (砾石>=0.3 & 经度<110°E) -> 降水[最值: {ctl_min_g:.2f} ~ {ctl_max_g:.2f} | 均值: {ctl_mean_g:.2f}] mm/day  |  偏差%[最值: {ctl_b_min_g:.2f}% ~ {ctl_b_max_g:.2f}% | 均值: {ctl_b_mean_g:.2f}%]")
    print(f"    - CTL CPL (华南 105-115°E, 20-25°N) -> 降水[最值: {ctl_min_sc:.2f} ~ {ctl_max_sc:.2f} | 均值: {ctl_mean_sc:.2f}] mm/day  |  偏差%[最值: {ctl_b_min_sc:.2f}% ~ {ctl_b_max_sc:.2f}% | 均值: {ctl_b_mean_sc:.2f}%]")
    print("")
    print(f"    - EXP CPL (砾石>=0.3 & 经度<110°E) -> 降水[最值: {exp_min_g:.2f} ~ {exp_max_g:.2f} | 均值: {exp_mean_g:.2f}] mm/day  |  偏差%[最值: {exp_b_min_g:.2f}% ~ {exp_b_max_g:.2f}% | 均值: {exp_b_mean_g:.2f}%]")
    print(f"    - EXP CPL (华南 105-115°E, 20-25°N) -> 降水[最值: {exp_min_sc:.2f} ~ {exp_max_sc:.2f} | 均值: {exp_mean_sc:.2f}] mm/day  |  偏差%[最值: {exp_b_min_sc:.2f}% ~ {exp_b_max_sc:.2f}% | 均值: {exp_b_mean_sc:.2f}%]")
    print("")
    print(f"    - (EXP-CTL) (砾石>=0.3 & 经度<110°E) -> 差值[最值: {diff_min_g:.2f} ~ {diff_max_g:.2f} | 均值: {diff_mean_g:.2f}] mm/day  |  偏差%差值[最值: {diff_b_min_g:.2f}% ~ {diff_b_max_g:.2f}% | 均值: {diff_b_mean_g:.2f}%]")
    print(f"    - (EXP-CTL) (华南 105-115°E, 20-25°N) -> 差值[最值: {diff_min_sc:.2f} ~ {diff_max_sc:.2f} | 均值: {diff_mean_sc:.2f}] mm/day  |  偏差%差值[最值: {diff_b_min_sc:.2f}% ~ {diff_b_max_sc:.2f}% | 均值: {diff_b_mean_sc:.2f}%]")
    print("")
    # ==========================================================
    ...
    if len(lon_sig) > 0:
         ax_comp.scatter(lon_sig, lat_sig, marker='.', color='black', s=5.5, alpha=0.8, transform=ccrs.PlateCarree(), zorder=2)
...
# ========================= 6. 保存 =========================
save_path = "../illustration/cpl/FIG6.precip_bias_percent_raw.pdf"
