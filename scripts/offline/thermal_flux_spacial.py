...
t_ctl_win_mon = f1_djf['f_t_soisno'].isel(soilsnow=slice(5, 15)).values
t_exp_win_mon = f2_djf['f_t_soisno'].isel(soilsnow=slice(5, 15)).values

n_years = 17
n_lat, n_lon, n_layers = t_ctl_mon.shape[1], t_ctl_mon.shape[2], 10

# 初始化季节平均数组
t_ctl_win, t_exp_win = np.zeros((n_layers, n_lat, n_lon)), np.zeros((n_layers, n_lat, n_lon))
...
for k in range(n_layers):
    for yr in range(n_years):
        start_win, end_win = 3 * yr, 3 * yr + 3
        start_spr, end_spr = 6 * yr, 6 * yr + 3
        start_sum, end_sum = 6 * yr + 3, 6 * yr + 6
        
        t_ctl_win[k] += np.nanmean(t_ctl_win_mon[start_win:end_win, :, :, k], axis=0) / n_years
        t_exp_win[k] += np.nanmean(t_exp_win_mon[start_win:end_win, :, :, k], axis=0) / n_years
...
theta_ctl_win = f3['theta_ctl_win_re'].values  
theta_exp_win = f3['theta_exp_win_re'].values
...
k_ctl_win, k_exp_win = np.zeros((10, n_lat, n_lon)), np.zeros((10, n_lat, n_lon))
...
for k_idx in range(10):
    ...
    # CTL
    Sr_c_win = calc_Sr(theta_ctl_win[k_idx], ths_c_interp[p])
    Sr_c_spr = calc_Sr(theta_ctl_spr[k_idx], ths_c_interp[p])
    Sr_c_sum = calc_Sr(theta_ctl_sum[k_idx], ths_c_interp[p])
    k_ctl_win[k_idx] = calc_k(t_ctl_win[k_idx], Sr_c_win, tkd_c_interp[p], tksu_c_interp[p], tksf_c_interp[p], vom_c_interp[p], vs_c_interp[p], vgr_c_interp[p], alf_c_interp[p], bet_c_interp[p])
    ...
    # EXP
    Sr_e_win = calc_Sr(theta_exp_win[k_idx], ths_e_interp[p])
    Sr_e_spr = calc_Sr(theta_exp_spr[k_idx], ths_e_interp[p])
    Sr_e_sum = calc_Sr(theta_exp_sum[k_idx], ths_e_interp[p])
    k_exp_win[k_idx] = calc_k(t_exp_win[k_idx], Sr_e_win, tkd_e_interp[p], tksu_e_interp[p], tksf_e_interp[p], vom_e_interp[p], vs_e_interp[p], vgr_e_interp[p], alf_e_interp[p], bet_e_interp[p])
...
# 计算界面热通量 F_i
F_ctl_win, F_exp_win = np.zeros((10, n_lat, n_lon)), np.zeros((10, n_lat, n_lon))
...
for i in range(9): 
    F_ctl_win[i] = calc_flux(k_ctl_win[i], k_ctl_win[i+1], z[i], z[i+1], zh[i], t_ctl_win[i], t_ctl_win[i+1])
    F_exp_win[i] = calc_flux(k_exp_win[i], k_exp_win[i+1], z[i], z[i+1], zh[i], t_exp_win[i], t_exp_win[i+1])
...
# 对各组的原始通量进行掩膜
F_ctl_win_ip = np.where(np.isnan(t_mask_3d) | (np.abs(F_ctl_win) < 1e-6), np.nan, F_ctl_win)
F_exp_win_ip = np.where(np.isnan(t_mask_3d) | (np.abs(F_exp_win) < 1e-6), np.nan, F_exp_win)
...
# 改为 10 行 6 列，调整 figsize (横向拉宽适应 6 列)
fig, axes = plt.subplots(10, 6, figsize=(28, 40), subplot_kw={'projection': proj})
plt.subplots_adjust(wspace=0.1, hspace=0.15, left=0.03, right=0.97, bottom=0.05, top=0.98)
...
# 组织 6 列的数据
plot_data = [
    (F_ctl_win_ip, "CTL DJF"),
    (F_exp_win_ip, "EXP DJF"),
...
for j in range(10):
    for i, (data_array, label_str) in enumerate(plot_data):
        ...
        # 提取当前图的字母（从 a 到 bh 共 60 个）
        letter = letters[j * 6 + i]
