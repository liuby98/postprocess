...
# 2. 逐年循环数据处理 (仅处理 U 风)
# ==========================================
def get_200hPa_data_manual(wrfout_file, pb_static):
    ...
    u_sum = np.zeros(var_U.shape[1:], dtype=np.float32)
    p_sum = np.zeros(var_P.shape[1:], dtype=np.float32)
    ...
    print(f"  [进程节点] {fname} 气候态U风极值: {np.nanmin(u_clim):.1f} to {np.nanmax(u_clim):.1f}")
    
    # 空间退交错
    u_unstag = 0.5 * (u_clim[..., :-1] + u_clim[..., 1:])
    p_tot_clim = (p_clim + pb_static) / 100.0

    print(f"  [进程节点] {fname} 正在进行 200 hPa 垂直插值...")
    u_200 = interplevel(u_unstag, p_tot_clim, 200.0)
    
    return u_200

def process_task(args):
    key, fpath, pb_static = args
    u_dat = get_200hPa_data_manual(fpath, pb_static)
    return key, u_dat
...
def main():
    ...
    # 西风急流核心风速色阶 (大于20m/s才上色)
    u_levels = np.arange(20, 55, 5) 
    # 差值色阶 (红色表示西风增强，蓝色表示减弱)
    diff_u_levels = np.arange(-1.5, 1.55, 0.1) 
    ...
    for i, panel in enumerate(panels):
        ...
        u_dat = data[panel["key"]]

        if panel["type"] == "abs":
            # 绝对值：只画西风急流的颜色阴影，使用 extend='max' 突出核心区
            cf = ax.contourf(lons, lats, u_dat, levels=u_levels, cmap='YlOrRd', 
                             transform=ccrs.PlateCarree(), extend='max', zorder=1)
            cf_abs = cf
        else:
            # 差值：绘制 U风 差异的颜色阴影
            cf = ax.contourf(lons, lats, u_dat, levels=diff_u_levels, cmap='RdBu_r', 
    ...
    cbar_abs.set_label('200 hPa U-Wind (m s$^{-1}$)', fontsize=13, fontweight='bold')
    
    # 差值 Colorbar
    cbar_ax_diff = fig.add_axes([0.25, 0.03, 0.5, 0.012])
    cbar_diff = fig.colorbar(cf_diff, cax=cbar_ax_diff, orientation='horizontal')
    cbar_diff.set_label('U-Wind Difference (m s$^{-1}$)', fontsize=13, fontweight='bold')

    # fig.suptitle("200 hPa Mean Westerly Jet (2001-2017)", fontsize=18, y=0.98, fontweight='bold')

    os.makedirs("./figs", exist_ok=True)
    save_name = "./figs/U_200hPa_Loop.pdf"
