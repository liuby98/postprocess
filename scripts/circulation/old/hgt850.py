...
# 850 hPa 特征等值线（可根据需要修改为 1460, 1520 等）
FEATURE_Z = 1520 
...
# ==========================================
# 2. 逐年循环数据处理 (850 hPa 位势高度)
# ==========================================
def get_850hPa_data_manual(wrfout_file, pb_static, phb_static):
    """
    切片计算每年的春季(MAM)或夏季(JJA)均值，求17年气候态后插值到850 hPa位势高度
    """
    ...
    # 修改为插值到 850.0 hPa
    z_850 = interplevel(z_unstag, p_tot_clim, 850.0)
    
    return z_850

def process_task(args):
    key, fpath, pb_static, phb_static = args
    z_dat = get_850hPa_data_manual(fpath, pb_static, phb_static)
    return key, z_dat
...
def main():
    ...
    # 等值线层级设置 (根据东亚 850hPa 高度场的典型范围调整: ~1350 到 ~1550 gpm)
    z_levels = np.arange(1360, 1560, 20)   
    diff_z_levels = np.arange(-20, 22, 2)  
    ...
    for i, panel in enumerate(panels):
        ...
        if panel["type"] == "abs":
            ...
            # 绘制 850 hPa 的特征线 (例如 1520 gpm)
            cs = ax.contour(lons, lats, z_dat, levels=[FEATURE_Z], colors='yellowgreen', 
            ...
            # 在差异图中画出 CTL 和 EXP 的特征线对比
            ctl_z = data[panel["ctl_key"]]
            exp_z = data[panel["exp_key"]]
            
            # CTL使用蓝色虚线
            ax.contour(lons, lats, ctl_z, levels=[FEATURE_Z], colors='blue', linestyles='--', 
                       linewidths=2.0, transform=ccrs.PlateCarree(), zorder=4)
            # EXP使用红色实线
            ax.contour(lons, lats, exp_z, levels=[FEATURE_Z], colors='red', linestyles='-', 
                       linewidths=2.0, transform=ccrs.PlateCarree(), zorder=4)
    ...
    cbar_abs.set_label('850 hPa Geopotential Height (gpm)', fontsize=13, fontweight='bold')
    ...
    fig.suptitle(f"850 hPa Geopotential Height and {FEATURE_Z} gpm contour (2001-2017)", fontsize=18, y=0.98, fontweight='bold')

    os.makedirs("./figs", exist_ok=True)
    save_name = "./figs/Geopotential_850hPa_Loop.png"
